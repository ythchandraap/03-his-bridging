...
export const getAllVisits = async (headers: any, connection: any) => {
  ...
  // if($this->session->userdata('paramedic_id') != '' && $this->session->userdata('paramedic_display_all_patient') == '0') {
  // 	$where .= " AND v.paramedic_id=".$this->session->userdata('paramedic_id')."";
  // }
  // if($this->config->item('prefix') == 'rspuraraharjamedika' && $this->session->userdata('paramedic_display_all_patient') == '1') {
  // 	$where .= " AND v.clinic_id=022";
  // }

  // $where .= ($param['schedule_id'] != '')?" AND v.schedule_id=".$param['schedule_id']." ":"";
  // $where .= " AND rc.`type` IN ('rawat jalan', 'ruang tindakan')";
  // $where .= ($param['clinic_id'] != '')?" AND rc.id=".$param['clinic_id']." ":"";
  // if(array_key_exists("payment_type_id", $param)){
  // 	$where .= ($param['payment_type_id'] != '')?" AND v.payment_type_id=".$param['payment_type_id']." ":"";
  // }
  // if(array_key_exists("batal", $param)){
  // 	$where .= ($param['batal'] == '1')?"":" AND (v.continue_id IS NULL OR v.continue_id !='10') ";
  // }else{
  // 	$where .= " AND (v.continue_id IS NULL OR v.continue_id !='10') ";
  // }
